...
import { Pencil, Check, X, BookMarked } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { toast } from "sonner";
import { updateLinkPanduanDosenAction } from "@/server/actions/kelas.actions";

interface PanduanDosenEditorProps {
  initialLink: string | null;
}

export default function PanduanDosenEditor({ initialLink }: PanduanDosenEditorProps) {
  ...
  function save() {
    startTransition(async () => {
      const result = await updateLinkPanduanDosenAction(draft);
      if (result.error) {
      ...
      setLink(draft.trim());
      setEditing(false);
      toast.success("Link panduan dosen diperbarui.");
    });
  }
    ...
    return (
      <div className="flex items-center gap-2 flex-wrap rounded-lg border border-primary/30 bg-primary/5 px-4 py-3">
        ...
        <div className="flex gap-2 shrink-0">
          <Button size="sm" className="gap-1.5 h-8" disabled={isPending} onClick={save}>
  ...
  if (!link) {
    return (
      ...
      >
        ...
        <span>Tambahkan panduan cara menggunakan aplikasi untuk dosen…</span>
      </button>
    );
  }
  ...
  return (
    <div className="flex items-center gap-3 rounded-lg border px-4 py-3">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-xs text-muted-foreground">Panduan Dosen</p>
