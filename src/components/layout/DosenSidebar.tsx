...
import { Users, LayoutDashboard, LogOut, Menu, X, GraduationCap, KeyRound } from "lucide-react"
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-2.5 px-5 py-5 border-b border-white/10">
        ...
        <div>
          <span className="font-bold text-white text-[15px] leading-tight block">Media Menulis</span>
          <span className="text-indigo-300/70 text-[11px]">LMS Menulis Esai</span>
        </div>
      </div>

      {/* Nav */}
      <nav className="flex-1 px-3 py-4 space-y-0.5">
        <p className="text-indigo-400/60 text-[10px] font-semibold uppercase tracking-wider px-3 mb-2">Menu</p>
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <Icon className={cn("h-4 w-4 shrink-0", isActive ? "text-indigo-300" : "text-indigo-400/60")} />
              {item.label}
              {isActive && (
      ...
      <div className="px-3 pb-4 border-t border-white/10 pt-3">
        <div className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-white/5 mb-1">
          ...
          <div className="min-w-0">
            <p className="text-sm font-medium text-white leading-none truncate">Dr. Rina Rosdiana</p>
