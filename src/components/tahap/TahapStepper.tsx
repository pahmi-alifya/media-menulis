      ...
      <div className="flex items-center justify-between rounded-lg border bg-muted/40 px-3 py-2 text-sm">
        <div className="flex flex-col space-y-1">
          ...
          <span className="text-xs text-muted-foreground">
            {unlockedCount} dari {tahapList.length} tahapan telah dibuka oleh
            dosen
          </span>
        </div>

        <div className="rounded-md bg-primary/10 px-2 py-1 text-xs font-semibold text-primary">
          {unlockedCount}/{tahapList.length} Terbuka
        </div>
      </div>
            ...
            return (
              <li key={tahap.id} className="flex items-start">
                ...
                <div className="flex flex-col items-center gap-1.5 w-[72px]">
                  ...
                  >
                    {TAHAP_LABEL[tahap.kode as keyof typeof TAHAP_LABEL]
                      ?.singkat ?? tahap.kode}
                  </span>
                </div>
