...
export default async function DosenDashboardPage() {
  ...
  const tahapTerbuka = kelas?.tahaps.filter((t: { isUnlocked: boolean }) => t.isUnlocked).length ?? 0
  ...
  return (
    <div className="p-6 md:p-8 space-y-8">
      ...
      <div>
        ...
        <h1 className="text-2xl font-bold">{namaDosen.split(",")[0]}</h1>
      </div>
        ...
        <>
          ...
          <div className="flex items-center gap-3 p-3 rounded-lg border bg-muted/30">
            <div className="flex-1 min-w-0">
              <p className="text-xs text-muted-foreground">Kode Kelas — {kelas.nama}</p>
              <p className="font-mono font-bold text-lg tracking-widest">{kelas.kode}</p>
            </div>
            <Badge variant="outline" className="text-xs shrink-0">Bagikan ke mahasiswa</Badge>
          </div>
